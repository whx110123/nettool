...
QString protocol103::protocol103wiscomnet::dealSControl()
{
	...
	tmp = CharToHexStr(m_apdu->apci.control[1]);
	if(m_apdu->apci.control[1] == 0x00)
	{
		text.append(tmp + "\t固定为0x00\r\n");
	}
	...
	tmp = CharToHexStr(m_apdu->apci.control[2]) + " " + CharToHexStr(m_apdu->apci.control[3]);
...
QString protocol103::protocol103wiscomnet::dealIControl()
{
	...
	tmp = CharToHexStr(m_apdu->apci.control[0]) + " " + CharToHexStr(m_apdu->apci.control[1]);
