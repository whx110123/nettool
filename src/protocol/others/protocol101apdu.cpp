...
QString protocol101::protocol101apdu::dealAPDU()
{
	QString text;
	QString tmp;
...
int protocol101::protocol101apdu::CharT0APDU(char *data, int len)
{
	if(len < 5)
	{
		return 1;
...
QString protocol101::protocol101apdu::dealERR(int err)
{
	QString text;
	switch(err)
	{
	...
	case 2:
		...
		break;
	case 3:
		text.append("出错！101报文必须是0x68或0x10开头，条件不满足，因此报文有问题\r\n");
		break;
	}
	return text;
}

QString protocol101::protocol101apdu::dealAPCI()
{
	QString text;
...
QString protocol101::protocol101apdu::dealCTRL()
{
	QString text = "控制域:";
...
QString protocol101::protocol101apdu::dealPRM()
{
...
QString protocol101::protocol101apdu::dealFCB()
{
...
QString protocol101::protocol101apdu::dealFCV()
{
...
QString protocol101::protocol101apdu::dealACD()
{
...
QString protocol101::protocol101apdu::dealDFC()
{
...
QString protocol101::protocol101apdu::dealCW1()
{
...
QString protocol101::protocol101apdu::dealCW2()
{
...
QString protocol101::protocol101apdu::dealCRC()
{
	QString text = "校验CRC:" ;
	uchar tmp = m_apdu->apci.ctrl + m_apdu->apci.addr + m_asdu->type + m_asdu->vsq ;
	for(int i = 0; i < cotlen; i++)
	{
		tmp += m_asdu->cot[i];
	}
	for(int i = 0; i < comaddrlen; i++)
	{
		tmp += m_asdu->commonaddr[i];
	}
	for(int i = 0; i < m_asdu->length - 2 - cotlen - comaddrlen; i++)
	{
		tmp += m_asdu->data[i];
	}
	if(m_apdu->crc == tmp)
